...
def get_example_images(dataset, test_list):
    ...
    for item in test_list:
        ...
        # 动态生成mask
        img_a = ((eyes_a.transpose(1,2,0) + 1) * 127.5).astype(np.uint8)
        img_b = ((eyes_b.transpose(1,2,0) + 1) * 127.5).astype(np.uint8)
        mask_a = dataset.get_eye_mask(img_a)
        boundary_mask_a = dataset.get_eye_mask_boundary(img_a)
        mask_b = dataset.get_eye_mask(img_b)
        entry = {
            ...
            'mask_a': torch.tensor(mask_a, dtype=torch.float),
            'boundary_mask_a': torch.tensor(boundary_mask_a, dtype=torch.float),
            'mask_b': torch.tensor(mask_b, dtype=torch.float),
        }
        entries.append(entry)
    test_visualize = {}
    for k in ['key', 'idx_a', 'idx_b', 'image_a', 'gaze_a', 'head_a', 'image_b', 'gaze_b', 'head_b', 'mask_a', 'boundary_mask_a', 'mask_b']:
        if k in entries[0]:
